...
class ThreadController extends Controller
{
    ...
    public function postThreadCommentsAction(Request $request, $id){
        ...
        if ($form->isValid()) {
            ...
            //return $this->redirect("http://fiter.local/app_dev.php/api/threads/type_294/comments");
            return $this->redirect( $this-> get('router')->generate('fos_comment_get_thread_comment', array('id' => $thread->getId(), 'commentId' => $comment->getId() )) );
    ...
    public function postThreadCommentVotesAction($id, $commentId)
    {
        ...
        $existingVote = $voteManager->findVoteById(array(
            'id' => $usr->getId(),
            'comment' => $comment->getId()
        ));
        if ($existingVote) {
            $this->context->addViolation('asd');
        }
